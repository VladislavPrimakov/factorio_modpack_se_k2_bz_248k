        ...
        for _, tech in pairs(data.raw["technology"]) do
            if tech.prerequisites then
                for pos, prerequisite in pairs(tech.prerequisites) do
                    if technology_name == prerequisite then
...
local function addEffect(technology_name, effect)
    if data.raw["technology"][technology_name] then
        if effect then
            if effect.type then
                if data.raw["technology"][technology_name].effects then
                    table.insert(data.raw["technology"][technology_name].effects, effect)
                else
                    data.raw["technology"][technology_name].effects = { effect }
                end
            end
        end
    end
